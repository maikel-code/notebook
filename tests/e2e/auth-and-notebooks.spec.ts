...
test("register, manage an own notebook, and sign out", async ({ page }) => {
  ...
  const createInput = page.getByLabel("Notebook-Name")
  ...
  await createInput.fill("Interview notes")
  await page.getByRole("button", { name: "Notebook anlegen" }).click()
  ...
  const renameInput = page.getByLabel("Neuer Notebook-Name")
  ...
  await renameInput.fill("Renamed notes")
  await page.getByRole("button", { name: "Umbenennen" }).click()
