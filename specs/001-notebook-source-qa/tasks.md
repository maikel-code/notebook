...
- [ ] T007 Skripte in `package.json` anlegen: `typecheck`, `lint`, `test`, `test:integration`, `test:e2e`, `db:reset`, `eval`, `worker:sweep` — Wortlaut und Torzuordnung nach plan.md, Abschnitt Verification Commands
...
- [ ] T014 Migration `sources` in `supabase/migrations/`: `storage_path` im Muster `{user_id}/{notebook_id}/{source_id}.pdf`, `byte_size` höchstens 25 MB, `page_count` höchstens 300, `status` aus `uploading` · `processing` · `ready` · `failed` · `unusable`, **Index auf `(notebook_id, content_hash)` bewusst nicht eindeutig**, weil FR-010a die zusätzliche Aufnahme einer inhaltsgleichen Datei erlaubt
...
- [ ] T021 [P] Grenzwerte an einer Stelle festhalten in `lib/limits.ts`: `MAX_FILE_BYTES` 25 MB, `MAX_PAGES` 300, `MAX_SOURCES_PER_NOTEBOOK` 50, `MAX_SELECTED_SOURCES` 10, `MAX_QUESTION_CHARS` 2.000, `MAX_CONTEXT_CHARS` 60.000, `MAX_JOB_ATTEMPTS` 3, `JOB_TIMEOUT_MS` 5 Minuten
...
- [ ] T024 Prüfgerüst für Integrationsläufe in `tests/integration/setup.ts`: zwei echte Benutzerkonten und ein anonymer Zugriff gegen die **lokale** Instanz, Aufräumen zwischen den Läufen
...
- [ ] T025 [P] [US1] Negativtest Zugriffsgrenze in `tests/integration/rls-notebooks.test.ts`: Konto B liest, ändert und löscht ein Notebook von Konto A — jeder Versuch scheitert
- [ ] T026 [P] [US1] Negativtest anonym in `tests/integration/rls-anonymous.test.ts`: Zugriff ohne Sitzung auf Notebook, Quelle und Datei liefert keine Inhalte
...
- [ ] T033 [US1] Server Actions `createNotebook`, `renameNotebook`, `deleteNotebook` in `app/notebooks/actions.ts` nach contracts/http-api.md; Name 1–200 Zeichen, nach Trimmen nicht leer
...
- [ ] T037 [P] [US2] Test Dateiannahme in `tests/integration/ingestion-rejects.test.ts`: beschädigtes PDF, leeres PDF, Bilddatei, passwortgeschütztes PDF, Datei über 25 MB, Dokument über 300 Seiten — jede landet sichtbar in Ablehnung oder Fehler, keine wird `ready` (SC-007)
...
- [ ] T041 [P] [US2] Test Wiederaufnahme in `tests/integration/ingestion-sweep.test.ts`: ein Auftrag mit `running` und überschrittener Laufzeitgrenze wird eingesammelt statt dauerhaft `processing` zu bleiben
...
- [ ] T043 [P] [US2] Test Zugriffsgrenze auf Dateien in `tests/integration/rls-storage.test.ts`: Konto B lädt die Datei von Konto A nicht herunter, auch nicht über den bekannten Pfad
...
- [ ] T047 [US2] Server Action `prepareUpload` in `app/notebooks/[notebookId]/actions.ts`: Prüfsumme des Inhalts abgleichen, `ok` · `duplicate` · `rejected` nach contracts/http-api.md, bei `intent: replace` die bisherige Quelle nach FR-031 entfernen
...
- [ ] T055 [US2] Quellenliste in `components/notebook/source-list.tsx` mit den Zuständen `wird verarbeitet`, `bereit`, `fehlgeschlagen`, `nicht nutzbar` und lesbarer Fehlerursache (FR-011, FR-012)
...
- [ ] T057 [P] [US2] Hochladefeld in `components/notebook/source-upload.tsx` mit Lade-, Erfolgs- und Fehlerzustand sowie der Rückfrage bei erkannter Dublette
...
- [ ] T061 [P] [US3] Test der Vorbedingungen in `tests/integration/chat-preconditions.test.ts`: keine Quelle ausgewählt, keine im Zustand `ready`, keine einschlägige Passage, Frage über 2.000 Zeichen — jeweils Erklärung statt Antwort mit Verweisen (FR-022)
- [ ] T062 [P] [US3] Test Ausfall des Antwortmodells in `tests/integration/chat-provider-failure.test.ts`: Nachricht endet auf `failed`, Teilantwort erscheint nicht als fertig, erneuter Versuch wird angeboten (FR-025)
- [ ] T063 [P] [US3] Ablauftest Kernablauf in `tests/e2e/core-flow.spec.ts`: anmelden, Notebook anlegen, PDF hochladen, fragen, Antwort lesen, Verweis öffnen und die hervorgehobene Passage sehen (SC-001, FR-032)
...
- [ ] T069 [US3] Antwortstrom in `app/api/chat/route.ts` über das Vercel AI SDK mit Anthropic Claude (D-04); nach Abschluss prüfen, Nachricht und geprüfte Verweise speichern
...
- [ ] T073 [US3] Belegansicht in `components/notebook/source-viewer.tsx` mit pdf.js: Seite öffnen und den geprüften Wortlaut in der Textebene hervorheben; wird er nicht gefunden, Seite öffnen und den Wortlaut daneben zeigen (D-11, FR-029)
...
- [ ] T079 [US4] Server Action `deleteSource` in `app/notebooks/[notebookId]/actions.ts`: mit Bestätigung, entfernt Datei und Abschnitte, lässt Verweise nach FR-031 unangetastet
...
- [ ] T091 [P] Bewertungslauf in `eval/run.ts`: Belegtreue und Anteil ehrlicher Einschränkungen am Referenzdatensatz, Bericht als Ausgabe; **kein Freigabetor** (Prinzip VI, plan.md)
- [ ] T092 [P] Referenzdatensatz vervollständigen in `eval/dataset/`: Dokumente mit bekannten Aussagen, Fragen mit erwarteten Belegstellen, Fragen ohne Beleglage; Umfang und Herkunft mit dem Maintainer klären (OD-03)
...
- [ ] T098 Alle Prüfkommandos aus plan.md einmal vollständig durchlaufen und das Ergebnis im Handoff festhalten (Gate 3, Gate 6)
...
US4, US5 und US6 hängen jeweils an US3, aber **nicht aneinander** — nach Abschluss von US3 sind sie parallel bearbeitbar.
...
**Zwei Regeln, die nicht verhandelbar sind**: Ein fehlschlagender Test wird nicht abgeschwächt, um eine Umsetzung durchzubringen (Prinzip I). Ein Datenpfad ohne Negativtest gilt als unfertig (Prinzip II, Gate 4).
